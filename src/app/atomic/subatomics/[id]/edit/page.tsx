...
const EditSubatomic: React.FC = () => {
  ...
  const handleSubmit = (data: SubatomicParticleUpdate) => {
    updateSubatomic(
      { id: particleId, data },
      {
        onSuccess: () => router.push(`/atomic/subatomics/${particleId}`),
        onError: (error: unknown) => {
          if (axios.isAxiosError(error) && error.response) {
            console.error("API Error updating subatomic particle:", error.response.data);
          } else {
            console.error("Error updating subatomic particle:", error instanceof Error ? error.message : "No additional details.");
          }
        },
      }
    );
