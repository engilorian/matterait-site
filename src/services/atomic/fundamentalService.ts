...
export const getFundamentalParticles = async (
  skip = 0,
  limit = 100
): Promise<FundamentalParticle[]> => {
  const response = await axios.get(atomicRoutes.fundamental.list, { params: { skip, limit } });
  return response.data;
};

export const getFundamentalParticleById = async (
  id: number
): Promise<FundamentalParticle> => {
...
export const createFundamentalParticle = async (
  data: FundamentalParticleCreate
): Promise<FundamentalParticle> => {
  const response = await axios.post(atomicRoutes.fundamental.create, data);
  return response.data;
};

export const updateFundamentalParticle = async (
  id: number,
  data: FundamentalParticleUpdate
): Promise<FundamentalParticle> => {
